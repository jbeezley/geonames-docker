#!/bin/bash
# adapted from https://github.com/jvarga/geonames-for-postgis
################################################################################
#   ____    ____    _____                                                      #
#  /\  _`\ /\  _`\ /\  __`\                                                    #
...
PGARGS="-U $DBUSER -h $DBHOST -p $DBPORT"
PSQL="psql -v ON_ERROR_STOP=1 $PGARGS"
...
$PSQL -e -c "CREATE DATABASE ${DBNAME} WITH TEMPLATE = template0 ENCODING = 'UTF8';" 
...
mkdir -p ${WORKPATH} || true
mkdir -p ${WORKDIR} || true
mkdir -p ${POSTALCODEDIR} || true
echo
...
createlang $PGARGS plpgsql ${DBNAME} || true
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/postgis.sql
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/postgis_comments.sql
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/spatial_ref_sys.sql
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/rtpostgis.sql
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/raster_comments.sql
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/topology.sql
$PSQL -d ${DBNAME} -f ${POSTGISPATH}/topology_comments.sql
